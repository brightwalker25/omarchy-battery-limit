...
Whether the cap survives a suspend at all depends on the model. On the laptop it was
written on it does, and the resume unit is a precaution that costs one file write.
...
The plugin has only ever been run on one machine, a laptop whose threshold
comes from a vendor WMI driver. The behaviour described below is taken from the code and
